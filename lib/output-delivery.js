 ...
 */
/**
 * Render an already-built section list. Callers that filter sections (the
 * digest drops filing changes the user has already been sent) use this so the
 * rendered body matches exactly the sections they kept — no second pass over
 * the data, and no chance of the email disagreeing with what was recorded.
 */
function renderSections(sections, opts = {}) {
    const base = baseUrl(opts.appUrl);
    const heading = opts.heading || '';
    if (!sections.length) return { sections, html: '', text: '', count: 0 };
    return {
        sections,
        count: sections.length,
        html: `<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:600px;margin:0 auto;color:#0f172a">`
...
module.exports = { renderBlock, renderSections, normalizeSymbols, TYPES };
