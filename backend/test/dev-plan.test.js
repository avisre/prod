...
test('the API landing page renders, prices the Dev plan, and carries the real example', () => {
    ...
    // The configs must point at the HOSTED endpoint. `stockportfolio-mcp` went
    // live on the registry 2026-09-14, so the assertion that used to guard
    // against advertising a 404ing package is now a positive one. The hosted URL
    // stays pinned regardless: every config here, npm bridge included, resolves
    // to it — the bridge is a transport hop, not a different backend.
    assert.match(html, /https:\/\/www\.stockportfolio\.pro\/mcp/, 'configs point at the hosted /mcp endpoint');
    assert.match(html, /stockportfolio-mcp/, 'advertises the published npm package');
    assert.match(html, /mcp-remote/, 'still offers the generic stdio bridge as an alternative');
