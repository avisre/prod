...
// decision log is append-only, positions are written exactly once (by the
// construction commit), the chat tool surface cannot trade, and credits
// meter rather than gate.
...
test('exactly three ai-paper routes exist, each behind auth + betaGate', () => {
    ...
    ].sort(), 'probe + create + detail, nothing else');
    for (const open of [
        "app.get('/api/ai-paper-portfolio',",
        "app.post('/api/ai-paper-portfolio/create',",
        "app.get('/api/ai-paper-portfolio/detail',"
...
test('positions are written by exactly one code path: the construction commit', () => {
    ...
    // Nothing else in the module (or app.js) sets positions in any write
    const writes = modSource.match(/\$set[^)]{0,300}\)/g) || [];
    const positionWrites = writes.filter((w) => /positions\s*:/.test(w));
    assert.equal(positionWrites.length, 1, 'no other $set ever contains positions');
...
test('the reset path only ever deletes the portfolio doc', () => {
    const reset = modSource.slice(modSource.indexOf('async function resetRun'), modSource.indexOf('async function resetRun') + 900);
    ...
    // failed builds self-clear so a retry is never 409-trapped — but only failed docs
    assert.match(modSource, /Portfolio\.deleteMany\(\{ user: userId, status: 'failed' \}\)/);
});

// ---- Chat tool surface ----

test('the ai-paper chat tools are exactly status/setup/reset — nothing that could trade', () => {
...
test('the chat tool handler can only read, kick a build, or reset', () => {
