...
<script src="/assets/app.js?v=20260831-uiaudit1"></script>
...
function notifySupportInbox(t) {
    try {
        ...
        const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#0f172a">`
            ...
            + `<tr><td style="padding:4px 12px 4px 0;color:#726e65">Name</td><td>${escapeHtml(t.name)}</td></tr>`
            + `<tr><td style="padding:4px 12px 4px 0;color:#726e65">Email</td><td><a href="mailto:${escapeHtml(t.email)}">${escapeHtml(t.email)}</a></td></tr>`
            + `<tr><td style="padding:4px 12px 4px 0;color:#726e65">Subject</td><td>${escapeHtml(t.subject)}</td></tr>`
            + `<tr><td style="padding:4px 12px 4px 0;color:#726e65">When</td><td>${when}</td></tr></table>`
...
function customerMessageMailFor(user, subject, body) {
    ...
    return {
        ...
        html: `<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:560px;margin:0 auto;color:#172033"><p>Hi ${escapeHtml(first)},</p><div style="white-space:normal;line-height:1.6">${safeBody}</div><p style="margin:22px 0"><a href="${escapeHtml(inboxUrl)}" style="background:#201f1d;color:#fff;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:700;display:inline-block">Reply in your private inbox</a></p><p style="font-size:12px;color:#726e65;line-height:1.55">This is a customer message from StockPortfolio.pro. <a href="${escapeHtml(unsubscribeUrl)}" style="color:#726e65">Stop customer-message emails</a>. In-app messages remain available in your account.</p></div>`
    };
}
...
app.get('/admin/marketing', authMiddleware, marketingDashboardOnly, async (req, res) => {
    ...
    try {
        ...
        const trendRows = data.trend.map((row) => {
            ...
            return `<tr><td>${e(row.day)}</td><td>${n(row.page_view)}</td><td>${bar(row.signup, '#5f5c55')}${n(row.signup)}</td><td>${bar(row.trial_start, '#4a4842')}${n(row.trial_start)}</td><td>${bar(row.paid, '#059669')}${n(row.paid)}</td><td>${bar(row.activation, '#d97706')}${n(row.activation)}</td></tr>`;
        }).join('');
