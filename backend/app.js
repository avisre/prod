...
async function collectMarketingDashboard() {
    ...
    return {
        ...
        researchRows,
        activationJobs,
        mcpCohort: await collectMcpCohort(col, now),
        trend: [...dayMap.values()]
    };
}

// Developer-funnel cohort, 7 days. Kept separate from the tables above because
// this funnel does not go through a browser at all — its population is chatbots
// and agent clients hitting POST /mcp, so it has no page views, no sessions and
// no signup step to slot into the conversion table.
//
// What matters here is one number: how many distinct keyless callers were
// REFUSED for wanting more than the free allowance. That is the only measured
// demand signal for the $19.99 Dev plan. Cap-hit events dedupe to one row per
// identity per reason per day, so these are distinct-caller-days, not raw
// refusals — a client hammering the endpoint counts once.
async function collectMcpCohort(col, now) {
    const since7 = new Date(now.getTime() - 7 * 86400000);
    const rows = await col.aggregate([
        // `timestamp`, not `createdAt` — this collection has no createdAt at
        // all, so that filter would match nothing and report zero demand
        // forever. logFunnelEvent writes `timestamp` on every event and it is
        // indexed. (capReason survives because FunnelEventSchema is strict:false.)
        { $match: { event: { $in: ['mcp_keyless_cap_hit', 'mcp_oauth_client_registered', 'mcp_oauth_authorized'] }, timestamp: { $gte: since7 } } },
        { $group: { _id: { event: '$event', reason: '$capReason' }, count: { $sum: 1 } } }
    ]).toArray().catch(() => []);

    const out = { capHits: 0, byReason: {}, clientsRegistered: 0, authorized: 0, since: since7.toISOString() };
    for (const r of rows) {
        const event = r._id && r._id.event;
        if (event === 'mcp_keyless_cap_hit') {
            out.capHits += r.count;
            const reason = (r._id && r._id.reason) || 'unknown';
            out.byReason[reason] = (out.byReason[reason] || 0) + r.count;
        } else if (event === 'mcp_oauth_client_registered') out.clientsRegistered += r.count;
        else if (event === 'mcp_oauth_authorized') out.authorized += r.count;
    }
    return out;
}
...
app.get('/admin/marketing', authMiddleware, marketingDashboardOnly, async (req, res) => {
    ...
    try {
        ...
        // Developer funnel. Deliberately its own panel rather than a row in the
        // conversion table: this traffic is chatbots and agent clients hitting
        // POST /mcp, with no page view, no session and no signup step to align
        // against. "Hit the free cap" is the number that matters — it counts
        // callers who wanted MORE than the free allowance, which is the only
        // measured demand the $19.99 Dev plan has ever had.
        const mcp = data.mcpCohort || { capHits: 0, byReason: {}, clientsRegistered: 0, authorized: 0 };
        const reasonNames = { ask_limit: 'Ran out of free questions', lookup_limit: 'Ran out of free lookups', global_ask_day: 'Hit the global daily ceiling' };
        const mcpReasonRows = Object.entries(mcp.byReason).sort((a, b) => b[1] - a[1])
            .map(([reason, value]) => `<tr><td>${e(reasonNames[reason] || reason)}</td><td>${n(value)}</td></tr>`).join('')
            || '<tr><td colspan="2">No keyless caller has hit the cap yet</td></tr>';
