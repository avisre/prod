...
const TOOL_DEFINITIONS = Object.freeze({
    ...
    'etf-overlap': { id: 'tool-etf-overlap', slug: 'etf-overlap', path: '/tools/etf-overlap', title: 'ETF Holdings Overlap', description: 'Compare the full portfolios of two ETFs and measure how much they overlap.', heading: 'ETF holdings overlap', intro: 'Enter two comma-separated ETF tickers to measure how much of each portfolio the two funds share.', multi: true },
...
async function computeMultiTool(tool, symbols) {
    if (tool === 'etf-overlap') {
        ...
        const [a, b] = profiles;
        // Full portfolios where both funds file N-PORT. This used to intersect
        // two ten-row Yahoo lists and present the result as overlap, which for
        // VOO vs VTI answered "10 shared holdings" against a real intersection
        // of ~500.
        const [fullA, fullB] = await Promise.all(symbols.map((symbol) => fundHoldings.fetchFundHoldings(symbol).catch(() => null)));
        const complete = !!(fullA && fullB);
        const listA = complete ? fullA.holdings : a.topHoldings;
        const listB = complete ? fullB.holdings : b.topHoldings;
        // CUSIP first: N-PORT names the same security differently across filers
        // ("NVIDIA Corp." vs "NVIDIA CORP"), and a name-only key undercounts.
        const keyOf = (row) => row.cusip || row.isin || row.symbol || fundHoldings.normalizeName(row.name);
        const bMap = new Map();
        for (const row of listB) { const key = keyOf(row); if (key && !bMap.has(key)) bMap.set(key, row); }
        const rows = listA.map((row) => {
            const other = bMap.get(keyOf(row));
            if (!other) return null;
            return {
                holding: row.symbol || row.name,
                firstWeightPct: row.weight == null ? null : row.weight * 100,
                secondWeightPct: other.weight == null ? null : other.weight * 100,
                minimumSharedWeightPct: row.weight != null && other.weight != null ? Math.min(row.weight, other.weight) * 100 : null
            };
        }).filter(Boolean).sort((x, y) => (y.minimumSharedWeightPct || 0) - (x.minimumSharedWeightPct || 0));
        const overlapPct = rows.reduce((sum, row) => sum + (row.minimumSharedWeightPct || 0), 0);
        const asOf = complete ? [fullA.asOf, fullB.asOf].filter(Boolean).join(' / ') : null;
        return {
            tool, symbols,
            source: complete ? 'SEC Form N-PORT' : 'Yahoo Finance fund profiles',
            generatedAt: new Date().toISOString(),
            summary: complete
                ? `${a.symbol} (${fullA.count} holdings) and ${b.symbol} (${fullB.count}) share ${rows.length} securities, ${overlapPct.toFixed(1)}% of assets by the lesser weight in each.`
                : `${a.symbol} and ${b.symbol} share ${rows.length} holdings within their reported top-holdings lists.`,
            metrics: [
                { label: complete ? 'Shared holdings' : 'Shared top holdings', value: rows.length, format: 'number' },
                { label: complete ? 'Portfolio overlap' : 'Minimum overlap shown', value: overlapPct, format: 'percent' }
            ],
            // The tail of a 500-way intersection is unreadable and would balloon
            // the page; the weights that matter are all at the top.
            rows: rows.slice(0, 100),
            warnings: complete
                ? [`Full portfolios as filed on Form N-PORT (${asOf}); N-PORT is public about 60 days after the reporting date, so both funds may have traded since.`]
                : ['At least one of these funds files no N-PORT, so this compares reported top holdings only and is a lower bound, not complete portfolio overlap.']
        };
    }
...
// NOTE: unused — module.exports below aliases the served `renderToolPage` to
// renderExpandedToolPage(). Kept only because other code in this file still
// calls it defensively; do not add tool-specific logic here, add it there.
...
function renderExpandedToolPage(tool) {
    ...
    // Insider Filing Explorer stays a genuine free SEC-link browser (6 most
    // recent Form 4s), but the full 3-year insider trail is Core — the CTA
    // here points there instead of the generic AppSumo funnel every other
    // free tool uses.
    const isInsiderFilings = tool === 'insider-filings';
    const ctaHref = isInsiderFilings
        ? '/register.html?plan=monthly'
        : `/go/appsumo/website?content_id=${encodeURIComponent(contentId)}`;
    const ctaLabel = isInsiderFilings ? 'Unlock the full 3-year insider trail with Core &rarr;' : 'Explore StockPortfolio.pro on AppSumo &rarr;';
    const ctaIntro = isInsiderFilings
        ? 'This list shows only the 6 most recent Form 4s. The full 3-year trail — every buy and sell, net shares per quarter, insider names — is included with Core.'
        : 'Continue with the complete source trail and research workflow.';
